...
/*
    metro_s = document.getElementById("map_metro_s");
    metromap = document.getElementById('metromap');
    a["metro[]"] = document.getElementById("map_metro[]");
    left_list = document.getElementById("map_metro_s");
    right_list = document.getElementById("map_metro[]");
...
for (i = 0; i < 300; i++)
    if (metro[i]) {
        ...
        mimg[i].src = "map/pp.gif";
        mimg[i].style.left = 509;
        mimg[i].style.top = 310;
...
function add_m() {
    ...
    if (left_sel != -1) {
        ...
        metro[src.value][1] = 1;
        if (mimg[src.value])
            metromap.appendChild(mimg[src.value]);
    }
}
 ...
 *
 * 
Эту переменную нужно в принципе убрать
 ***/
//var map_sel = 22;
